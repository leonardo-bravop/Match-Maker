...
const ItemRecord = ({ item, setOpenedMatch, openedMatch }) => {
  ...
  useEffect(() => {
    ...
    if (item.status === "completada") {
      axios
        ...
        .then(({ data }) => {
          if (
            item.team_1.filter((element) => element._id === user._id).length ===
            1
          ) {
    ...
    if (
      ...
      moment().isSameOrAfter(
        moment(`${item.date} ${item.time}`, "DD-MM-YYYY H:mm")
      )
    ) {
      item.status = "completada";
    }
  ...
  const acceptHandler = () => {
    axios
      .put(
        `${uri}/api/invitation/invitAcepted/${item._id}/user/${user._id}`,
        {},
  ...
  const resultHandler = () => {
    let finalScore;
    if (
      item.team_1.filter((element) => element._id === user._id).length === 1
    ) {
    ...
    axios
      .put(
        `${uri}/api/result/updateResult/match/${item._id}/user/${user._id}`,
        { score: finalScore },
        { headers: { Authorization: `Bearer ${userString}` } }
      )
      .then(({ data }) => {
  ...
  const cancelHandler = () => {
    axios
      .put(
        `${uri}/api/invitation/invitRejected/${item._id}/user/${user._id}`,
        {},
        { headers: { Authorization: `Bearer ${userString}` } }
      )
      .then(({ data }) => {
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        itemStyles.item,
        { borderColor: colorSet[item.status], borderWidth: 1.5 },
      ]}
    >
      <View style={[itemStyles.head]}>
        <View
          style={[itemStyles.team, { borderColor: "blue", borderWidth: 0 }]}
        >
          {item[
            item.team_1.filter((element) => element._id === user._id).length ===
            1
        ...
        <View style={[itemStyles.info, { borderColor: "red", borderWidth: 0 }]}>
          {item.status === "win" || item.status === "lost" ? (
            <Text
              style={[
                itemStyles.text,
                { fontSize: 25, color: colorSet[item.status] },
              ]}
            >
              {`${score[0]} - ${score[1]}`}
            </Text>
            ...
            <>
              <Text
                style={[
                  itemStyles.text,
                  { textTransform: "uppercase", color: colorSet[item.status] },
                ]}
              >
        ...
        <View>
          ...
          >
            <Text style={[itemStyles.text]}>
              El partido se{" "}
              {item.status === "pendiente" || item.status === "lista"
                ? "disputara"
                : "disputo"}{" "}
              a las {item.time}
            </Text>
            {item.invitationText === "" ? (
              ...
              <Text style={[itemStyles.text]}>{item.invitationText}</Text>
            )}
          </View>
                ...
                <View style={{ marginTop: 30 }}>
                  <TouchableOpacity
                    style={[
                      ...
                      { backgroundColor: colorSet[item.status] },
                    ]}
          ...
          {item.status === "lista" ? (
            <View>
              ...
              <View style={{ marginVertical: 10, flexDirection: "row" }}>
                ...
                >
                  <Text
                    style={[cardStyles.cancelTxt, { color: colorSet.error }]}
                  >
                    {"Cancelar"}
                  </Text>
                </TouchableOpacity>
